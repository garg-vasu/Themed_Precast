  ...
  useEffect(() => {
    ...
    const fetchWarehouses = async () => {
      try {
        ...
        if (!axios.isCancel(err)) {
          toast.error(getErrorMessage(err, "warehouses data"));
        }
      }
    };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (isEditMode && user) {
        ...
        const response = await apiClient.put(`/client_update`, payload);
        if (response.status === 200 || response.status === 201) {
        ...
        // Create new tenant
        const response = await apiClient.post("/client_create", payload);
        if (response.status === 200 || response.status === 201) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          ...
          <div className="grid w-full items-center gap-1.5">
            <Label htmlFor="store_id">
              Store/Warehouse <span className="text-red-500">*</span>
            </Label>
            <Controller
              control={control}
              name="store_id"
              render={({ field }) => (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Stores/Warehouses</SelectLabel>

...
const STEPS = [
  { label: "Select Element Types", description: "Choose one or more element types" },
...
export default function EditbomElemenrtype() {
  ...
  // Quantities: Map<element_type_id, Map<bom_id, quantity>>
  const [quantities, setQuantities] = useState<Map<number, Map<number, number>>>(new Map());
    ...
    const fetchElementTypes = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          toast.error(response.data?.message || "Failed to fetch element types");
        }
      } catch (err: unknown) {
  ...
  const toggleAllElements = (checked: boolean) => {
    if (checked) {
      setSelectedElementTypeIds(new Set(filteredElements.map((e) => e.element_type_id)));
    } else {
  ...
  const initializeQuantities = () => {
    const newQuantities = new Map<number, Map<number, number>>();
    selectedElementTypeIds.forEach((etId) => {
      const bomMap = new Map<number, number>();
      selectedBomIds.forEach((bomId) => {
        // Preserve existing quantity if any
        const existing = quantities.get(etId)?.get(bomId);
        bomMap.set(bomId, existing ?? 1);
      });
      newQuantities.set(etId, bomMap);
    });
    setQuantities(newQuantities);
  };

  const updateQuantity = (etId: number, bomId: number, value: number) => {
    setQuantities((prev) => {
      const next = new Map(prev);
      const bomMap = new Map(next.get(etId) || new Map());
      bomMap.set(bomId, Math.max(0, value));
      next.set(etId, bomMap);
      return next;
    });
  };
  ...
  const validateQuantities = (): boolean => {
    let valid = true;
    quantities.forEach((bomMap, _etId) => {
      bomMap.forEach((qty, _bomId) => {
        if (qty <= 0 || isNaN(qty)) {
          valid = false;
        }
      });
  ...
  const handleSubmit = async () => {
    ...
    quantities.forEach((bomMap, etId) => {
      const bomItems: BomQuantityEntry[] = [];
      bomMap.forEach((qty, bomId) => {
        bomItems.push({ bom_id: bomId, quantity: qty });
      });
      assignments.push({ element_type_id: etId, bom_items: bomItems });
    });
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        toast.error(response.data?.message || "Failed to save BOM assignments.");
      }
    } catch (err: unknown) {
  ...
  // ── Lookup helpers ─────────────────────────────────────────────────
  const getElementTypeName = (id: number) => {
    const et = elementTypes.find((e) => e.element_type_id === id);
    return et ? `${et.element_type} — ${et.element_type_name}` : `Element ${id}`;
  };
            ...
            {loadingElements ? (
              ...
              <>
                ...
                <div className="flex items-center gap-2 py-2 px-3 rounded-md bg-muted/50">
                  ...
                  />
                  <Label htmlFor="select-all-elements" className="cursor-pointer text-sm font-medium">
                ...
                <ScrollArea className="h-[360px] sm:h-[420px]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {filteredElements.map((et) => {
                      const isChecked = selectedElementTypeIds.has(et.element_type_id);
                      return (
                        <div
      ...
      {currentStep === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose BOMs to assign to the {selectedElementTypeIds.size} selected element type(s).
              <Badge variant="secondary" className="ml-2">
              ...
              <>
                ...
                <div className="flex items-center gap-2 py-2 px-3 rounded-md bg-muted/50">
                  <Checkbox
                    ...
                    checked={
                      filteredBoms.length > 0 &&
                      filteredBoms.every((b) =>
                        selectedBomIds.has(b.bom_id),
                      )
                    }
                    onCheckedChange={(checked) => toggleAllBoms(!!checked)}
                  />
                  <Label htmlFor="select-all-boms" className="cursor-pointer text-sm font-medium">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter the quantity for each BOM item per element type.
              </CardDescription>
            </CardHeader>
          </Card>

          <ScrollArea className="h-[500px] sm:h-[560px]">
            <div className="space-y-4 pr-3">
              {Array.from(selectedElementTypeIds).map((etId) => (
                <Card key={etId}>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base">
                      {getElementTypeName(etId)}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {Array.from(selectedBomIds).map((bomId) => {
                        const qty = quantities.get(etId)?.get(bomId) ?? 1;

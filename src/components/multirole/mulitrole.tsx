...
export default function MultiRole({
  ...
  const availableUsers = useMemo(() => {
    return users.filter(
      (user) =>
        !selectedUsers.includes(user.role_id) &&
  ...
  const handleSelectUser = (user: Role) => {
    if (!selectedUsers.includes(user.role_id)) {
      const updatedSelected = [...selectedUsers, user.role_id];
  ...
  const handleRemoveUser = (roleId: number) => {
    const updatedSelected = selectedUsers.filter((id) => id !== roleId);
    setSelectedUsers(updatedSelected);
    onSelectionChange(updatedSelected);
  };
      ...
      >
        {selectedUsers.length > 0 && (
          <div className="flex flex-row flex-wrap items-center gap-1.5 w-full">
            {selectedUsers.map((roleId) => {
              const user = users.find((user) => user.role_id === roleId);
              return (
                user && (
                  <Pills
                    key={`pill-${roleId}`}
                    text={user.role_name}
                    onClick={() => handleRemoveUser(roleId)}
                  />
                )
              );
            })}
        ...
        <div className="flex items-center gap-1.5 w-full">
          ...
          <Input
            ...
            type="text"
            placeholder={
              selectedUsers.length === 0 ? placeholder : "Search roles..."
            }
            value={searchQuery}
